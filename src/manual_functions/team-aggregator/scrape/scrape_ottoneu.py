...
class Scrape_Ottoneu(scrape_base.Scrape_Base):
    ...
    def scrape_league_table(self) -> DataFrame:
        ...
        url = 'https://ottoneu.fangraphs.com/ajax/browseleagues'
        response = requests.get(url)
        df = DataFrame(response.json())
        df.set_index('ID')
        return df

    def scrape_league_table_old(self) -> DataFrame:
        """Scrapes the Ottoneu Browse Leagues page to return DataFrame with available information for all active leagues. Index is League Id"""
    ...
    def __parse_standings_rank_table(self, table_sects: ResultSet) -> DataFrame:
        ...
        for section in table_sects:
            ...
            if (
                header is not None
                and header.contents is not None
                and len(header.contents) > 0
                and header.contents[0].string == 'Rankings'
            ):
                sect = section
                break
        if sect is None:
    ...
    def __parse_standings_pt_table(self, table_sects: ResultSet) -> DataFrame:
        ...
        for section in table_sects:
            ...
            if (
                ...
                and header.contents is not None
                and len(header.contents) > 0
                and header.contents[0].string == 'Games Played and Innings Pitched'
            ):
                sect = section
                break
        if sect is None:
        ...
        for td in tds[1:]:
            try:
                if len(td.find_all('strong')) > 0:
                    ...
                    len(td.find_all('span', {'class': 'negative-change-from-yesterday'})) > 0
